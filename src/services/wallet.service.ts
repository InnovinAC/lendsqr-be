...
class WalletService extends Service {
  ...
  private async addTransaction(data: Partial<Transaction>, transaction?: Knex.Transaction) {
    (transaction || this.db).table<Transaction>(TableName.TRANSACTIONS).insert(data);
  }
  ...
  private async increaseWalletBalance(
    ...
    (transaction || this.db).table(TableName.WALLET).where('id', id).increment({
      balance: amount,
    });
  }
  ...
  private async decreaseWalletBalance(
    ...
    (transaction || this.db).table(TableName.WALLET).where('id', id).decrement({
      balance: amount,
    });
  }
